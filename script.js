...
class App {
  ...
  _getPosition() {
    if (navigator.geolocation)
      navigator.geolocation.getCurrentPosition(
        ...
        function () {
          alert('Not able to access the location');
        }
      );
  }
  _loadMap(position) {
    ...
    this.#workouts.forEach(work => this._renderWorkoutMarkup(work));
  }
  _showForm(mapE) {
    this.#mapEvent = mapE;
  ...
  _newWorkout(e) {
    ...
    if (type === 'running') {
      const cadence = +inputCadence.value; // converting string to number.
      if (
        !allNumbers(distance, duration, cadence) ||
        !allPositives(distance, duration, cadence)
      )
        alert('It should be a positive number');
      workout = new Running([lat, lng], distance, duration, cadence);
    }
    if (type === 'cycling') {
      const elevation = +inputElevation.value; // converting string to number.
      if (
